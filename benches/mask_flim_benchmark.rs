...
/// (to compare overhead latency) and then many frames with and without registration
/// (to compare the actual effect of adding registration)
fn criterion_benchmark_read_one_phasor_mask(c: &mut Criterion) {

    //////////////////////// FLAT MASKS/////////////////
    let siffreader = corrosiff::open_siff(SHORT_SIFF_PATH).unwrap();
    let mut read_bench = c.benchmark_group("Phasor Mask sum benchmarks");
    let frame_vec = Vec::<u64>::from_iter(0..40);
    ...
    read_bench.bench_with_input(
        ...
        &frame_vec.as_slice(),
        |bench, frames| {
            bench.iter(|| black_box(siffreader.sum_roi_phasor_flat(&mask.view(), frames, None).unwrap()))
        },
    );
    ...
    read_bench.bench_with_input(
        ...
        &frame_vec.as_slice(),
        |bench, frames| {
            bench.iter(|| black_box(siffreader.sum_roi_phasor_flat(&mask.view(), frames, Some(&reg)).unwrap()))
        },
    );
    ...
    read_bench.bench_with_input(
        ...
        &frame_vec.as_slice(),
        |bench, frames| {
            bench.iter(|| black_box(siffreader.sum_roi_phasor_volume(&mask.view(), frames, None).unwrap()))
        },
    );
    ...
    read_bench.bench_with_input(
        ...
        &frame_vec.as_slice(),
        |bench, frames| {
            bench.iter(|| black_box(siffreader.sum_roi_phasor_volume(&mask.view(), frames, Some(&reg)).unwrap()))
        },
    );
...
///// Multimasks!!////
fn criterion_benchmark_read_multiple_phasor_masks(c: &mut Criterion) {

    //////////////////////// FLAT MASKS/////////////////
    let siffreader = corrosiff::open_siff(SHORT_SIFF_PATH).unwrap();
    let mut read_bench = c.benchmark_group("Multimask Phasor sum benchmarks");
    let frame_vec = Vec::<u64>::from_iter(0..40);
    ...
    read_bench.bench_with_input(
        ...
        &frame_vec.as_slice(),
        |bench, frames| {
            bench.iter(|| black_box(siffreader.sum_rois_phasor_flat(&masks.view(), frames, None).unwrap()))
        },
    );
    ...
    read_bench.bench_with_input(
        ...
        |bench, frames| {
            bench.iter(||
                black_box(
                    masks.axis_iter(Axis(0)).for_each(|mask| {
                        siffreader.sum_roi_phasor_flat(&mask, frames, None).unwrap();
                    })
                )
            )
        },
    ...
    read_bench.bench_with_input(
        ...
        &frame_vec.as_slice(),
        |bench, frames| {
            bench.iter(|| black_box(siffreader.sum_rois_phasor_flat(&masks.view(), frames, Some(&reg)).unwrap()))
        },
    );
    ...
    read_bench.bench_with_input(
        ...
        |bench, frames| {
            bench.iter(||
                black_box(
                    masks.axis_iter(Axis(0)).for_each(|mask| {
                        siffreader.sum_roi_phasor_flat(&mask, frames, Some(&reg)).unwrap();
                    })
                )
            )
        },
    ...
    read_bench.bench_with_input(
        ...
        &frame_vec.as_slice(),
        |bench, frames| {
            bench.iter(|| black_box(siffreader.sum_rois_phasor_volume(&masks.view(), frames, None).unwrap()))
        },
    );
    ...
    read_bench.bench_with_input(
        ...
        &frame_vec.as_slice(),
        |bench, frames| {
            bench.iter(|| black_box(siffreader.sum_rois_phasor_volume(&masks.view(), frames, Some(&reg)).unwrap()))
        },
    );
