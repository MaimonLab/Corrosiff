...
pub fn _load_flim_array_empirical_compressed<T: Into<u64>>(
    ...
    intensity.iter().zip(array.iter_mut()).for_each(|(intensity, pixel)| {
        arrival_times[
            arrival_time_pointer..
            arrival_time_pointer+*intensity as usize].iter().for_each(|x| {
            *pixel += *x as f64;
        });
...
pub fn _load_flim_array_empirical_uncompressed<T : Into<u64>>(
    ...
    intensity.iter().zip(array.iter_mut())
    .for_each(|(intensity, pixel)| {
        *pixel /= *intensity as f64;
    });
...
pub fn _load_flim_intensity_empirical_compressed<T : Into<u64>>(
    ...
    // increments through arrival_times with intensity
    let mut arrival_time_pointer : usize = 0;
    intensity_array.iter().zip(lifetime_array.iter_mut())
    .for_each(|(intensity, pixel)| {
        arrival_times[
            arrival_time_pointer..
            arrival_time_pointer+*intensity as usize
...
#[binrw::parser(reader)]
pub fn _extract_mask_empirical_intensity_raw<T : Into<u64>>(
    mask : &ArrayView2<bool>,
    lookup_table : &ArrayView2<usize>,
    lifetime_array : &mut ArrayViewMut1<f64>,
    ...
    photonwise_op!(
        ...
        |siffphoton| {
            let y = photon_to_y!(*siffphoton, 0, ydim);
            let x = photon_to_x!(*siffphoton, 0, xdim);
            let px = lookup_table[[y, x]];
            lifetime_array[px] += photon_to_tau_FLOAT!(*siffphoton)
                * (mask[[y, x]] as u64 as f64);
            intensity_array[px] += mask[[y, x]] as u16;
        }
    );
    Ok(())
}
...
pub fn _sum_masks_empirical_intensity_compressed<T : Into<u64>>(
    ...
    izip!(
        ...
        let mut lifetime_pointer : usize = 0;
        intensity_data.iter().zip(mask.iter()).for_each(|(intensity, maskpx)| {
            *intensity_accumulator += (*intensity as u64)
                * (*maskpx as u64);
...
#[binrw::parser(reader)]
pub fn _extract_mask_empirical_intensity_compressed<T : Into<u64>>(
    mask : &ArrayView2<bool>,
    lookup_table : &ArrayView2<usize>,
    lifetime_array : &mut ArrayViewMut1<f64>,
    intensity_sum : &mut ArrayViewMut1<u16>,
    strip_byte_counts : T,
    ydim : u32,
    xdim : u32,
) -> Result<(), CorrosiffError> {
    ...
    izip!(
        ...
        lookup_table.iter(),
        intensity_data.iter(),
    ).for_each(|(&mask, &lookup, &intensity)| {
            intensity_sum[lookup] += mask as u16 * intensity;
            lifetime_array[lookup] += 
                lifetime_data[
                    lifetime_pointer..
                    lifetime_pointer+intensity as usize
                ].iter().sum::<u16>() as f64;
